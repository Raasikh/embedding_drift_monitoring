...
> Real-world data shifts but model embeddings stay frozen from training. The gap grows, causing silent retrieval quality degradation.

**Q: How did you detect it?**
> Scheduled monitoring pipeline computing centroid shift, KL divergence, neighborhood stability, per-item cosine drift, and recall@K.

**Q: What were the retraining triggers?**
> Three types: threshold-based, rate-of-change, and composite. Built-in cooldown prevents trigger storms. Fires kick off SageMaker training jobs.

**Q: How did you measure 32%?**
> Compared cumulative recall loss: fixed-schedule retraining vs automated monitoring. Monitored system caught drift early, reducing total degradation by ~32%.

**Q: Why not just retrain on a fixed schedule?**
> Two problems: wastes compute when no drift, and misses sudden events between schedules. Our system caught a distribution shift within one step that fixed-schedule wouldn't catch for 8+ steps.

**Q: What types of drift did you handle?**
> Gradual (slow behavior change), sudden (new product launch), and seasonal (holiday patterns). Each needs different detection sensitivity.

**Q: What tools in production?**
> Airflow DAGs for metrics, Prometheus + Grafana dashboards, SageMaker for training, FAISS for vector index. Considered Arize Phoenix and Evidently AI.
